...
import {
  ...
  Search, MapPin, User, Sunrise, Sunset, Plus, CloudSun, MessageSquare, Layers, Crosshair, CloudFog, Moon, Bell, Eye, Lock, Crown
} from 'lucide-react';
...
const PremiumValue = ({ isLocked, children }: { isLocked: boolean, children: React.ReactNode }) => {
  ...
  return (
    <div className="relative group cursor-pointer w-full" onClick={() => alert("Upgrade to Premium to see detailed data!")}>
...
const WeatherDashboard = () => {
  const { weather, loadingWeather, unit, t, cityName, language, userTier } = useContext(AppContext)!;
      ...
      <div className="border-t border-gray-100 pt-6">
        ...
        <div className="flex overflow-x-auto gap-8 pb-2 scrollbar-hide">
          ...
          {userTier === UserTier.FREE && displayTimes.length > 3 && (
            <div className="flex flex-col items-center justify-center gap-2 flex-shrink-0 min-w-[3rem] opacity-60 cursor-pointer group" onClick={() => alert("Upgrade to see 12h forecast!")}>
...
const App = () => {
  ...
  const { language, setLanguage, unit, setUnit, t, requestNotifications, notificationsEnabled, testPush, lastNotification, user, userTier } = useContext(AppContext)!;
        ...
        <div className="flex gap-2">
          {userTier === UserTier.FREE && (
            <button
              ...
              className="w-8 h-8 rounded-full bg-yellow-50 border border-yellow-200 text-yellow-600 flex items-center justify-center hover:bg-yellow-100 mr-1 relative overflow-hidden group"
            >
